...
| cleaning   | `line_length_bimodality` | 0..1, >0.6 hints at columns (calibrated on 118 single-column CVs: bulk 0.48–0.59, outliers 0.61+) |
...
### 3. Two-column reflow
When `line_length_bimodality > 0.6` we currently just flag it; the text still arrives in interleaved column order and segmentation suffers. Plan: post-process pdfplumber chars by x-clustering into column bands, re-emit the text in column-then-row order before handing to `clean_text`.
